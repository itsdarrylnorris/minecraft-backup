...
if screen -list | grep -q "dominecraft"; then
    echo "Server is already started!  Press screen -r dominecraft to open it"
    exit 1
fi
...
cd /root/minecraftbe/dominecraft

# Create backup
if [ -d "worlds" ]; then
    echo "Backing up server (to minecraftbe/dominecraft/backups folder)"
...
echo "Starting Minecraft server.  To view window type screen -r dominecraft"
echo "To minimize the window and let the server run in the background, press Ctrl+A then Ctrl+D"
screen -dmS dominecraft /bin/bash -c "LD_LIBRARY_PATH=/root/minecraftbe/dominecraft /root/minecraftbe/dominecraft/bedrock_server"
